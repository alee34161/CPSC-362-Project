...
const Cafe: React.FC = () => {
	...
	const fetchSearchResults = async (query: string) => {
		...
		setIsLoading(true);
		try {
			const response = await axios.get(`http://localhost:8080/allmenusearch`, {
				params: { query } // Adjust the API endpoint as needed
			});
			setResults(response.name); // Update with actual data shape
			} catch (error) {
            ...
            <header>
                <div className="navbar-container">
                    ...
                    <div className="search-container">
                        <input type="search" placeholder="Search for something..." value={ searchTerm } onChange={ handleSearchChange }/>
                                          ...
                                          <ul className="search-results">
                                              {isLoading ? (
                                                  ...
                                                  results.map((result, index) => (
                                                      <li key={index}>{result.name} href='/${result.source}menu'</li>
                                                  ))
                                              ) : (
